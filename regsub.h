...
namespace mfl{
    ...
    std::pair<int, int> regSubDP(const std::string& reg, const std::string& word)  {
        ...
        std::vector<char> goodLetters = {'a', 'b', 'c'};
        std::vector<char> goodSymbols = {'1', '.', '+', '*'};
        for (size_t i = 0; i < word.size(); ++i) {
            if (!find(goodLetters, word[i])) {
                throw std::runtime_error("illegal string.");
            }
        }
        for (size_t i = 0; i < reg.size(); ++i) {
            if (!find(goodLetters, reg[i]) && !find(goodSymbols, reg[i])) {
                throw std::runtime_error("illegal reg.");
            }
        }
        for (size_t i = 0; i < reg.size(); ++i) {
            if (std::find(goodLetters.begin(), goodLetters.end(), reg[i]) != goodLetters.end()) {
